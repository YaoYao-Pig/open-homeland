Shader "Planet/AudioReactiveAtmosphere"
{
    Properties
    {
        // 基本属性
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                float wave = sin(time + worldPos.x * _WaveFrequency) * 
                             sin(time + worldPos.y * _WaveFrequency) * 
                             sin(time + worldPos.z * _WaveFrequency);
                
                // 应用音频脉冲调制波动
                float audioWave = wave * _WaveAmplitude * _AudioPulse * _PulseIntensity;
